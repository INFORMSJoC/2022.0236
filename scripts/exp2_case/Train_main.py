...
def offline_stage(n_site, budget, iter_num):
    ...
    if istrain == 0:
        agent = []
        for h in hyper:
            temp = Agent(state_size=n_state, action_size=n_action, initialize=2,
                         noise=h[0], lr=h[1], units=h[2], layer=h[3], site=n_site)
            agent.append(temp)
    else:
        agent = load('network_initialization/init_' + str(n_site), n_state, n_action, 54, n_site)
    ...
    save_training(agent, 'result/transferRL_' + str(round(n_site)) + 'patch')
